...
if [ -n "${BACKUP_RCLONE_REMOTE:-}" ]; then
  ...
  echo "[backup] BACKUP_RCLONE_REMOTE not set — dumps stay local only, see README" >&2
fi

RETENTION_DAYS="${BACKUP_RETENTION_DAYS:-7}"
find "$DUMP_DIR" -type f -mtime +"$RETENTION_DAYS" -delete

# Healthcheck freshness marker (see compose.yaml) — touched once the dump
# (and, if configured, its off-server shipment) has completed, so a
# stuck/failing job (this script runs under `set -eu`, so any of the steps
# above failing exits here first) shows up as unhealthy instead of a
# silently stale local dump directory.
